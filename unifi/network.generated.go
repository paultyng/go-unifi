...
func (dst *Network) UnmarshalJSON(b []byte) error {
	...
	dst.DHCPDLeaseTime = aux.DHCPDLeaseTime.Value()
	dst.DHCPDTimeOffset = aux.DHCPDTimeOffset.Value()
	dst.DHCPDV6LeaseTime = aux.DHCPDV6LeaseTime.Value()
	dst.IGMPGroupmembership = aux.IGMPGroupmembership.Value()
	dst.IGMPMaxresponse = aux.IGMPMaxresponse.Value()
	dst.IGMPMcrtrexpiretime = aux.IGMPMcrtrexpiretime.Value()
	dst.IPSecDhGroup = aux.IPSecDhGroup.Value()
	dst.IPSecEspDhGroup = aux.IPSecEspDhGroup.Value()
	dst.IPSecIkeDhGroup = aux.IPSecIkeDhGroup.Value()
	dst.IPV6RaPreferredLifetime = aux.IPV6RaPreferredLifetime.Value()
	dst.IPV6RaValidLifetime = aux.IPV6RaValidLifetime.Value()
	dst.InterfaceMtu = aux.InterfaceMtu.Value()
	dst.InternetAccessEnabled = emptyBoolToTrue(aux.InternetAccessEnabled)
	dst.LocalPort = aux.LocalPort.Value()
	dst.OpenVPNLocalPort = aux.OpenVPNLocalPort.Value()
	dst.OpenVPNRemotePort = aux.OpenVPNRemotePort.Value()
	dst.PptpcRouteDistance = aux.PptpcRouteDistance.Value()
	dst.Priority = aux.Priority.Value()
	dst.RouteDistance = aux.RouteDistance.Value()
	dst.UidPublicGatewayPort = aux.UidPublicGatewayPort.Value()
	dst.UidVPNMaxConnectionTimeSeconds = aux.UidVPNMaxConnectionTimeSeconds.Value()
	dst.VLAN = aux.VLAN.Value()
	dst.VrrpVrid = aux.VrrpVrid.Value()
	dst.WANDHCPCos = aux.WANDHCPCos.Value()
	dst.WANDHCPv6PDSize = aux.WANDHCPv6PDSize.Value()
	dst.WANEgressQOS = aux.WANEgressQOS.Value()
	dst.WANLoadBalanceWeight = aux.WANLoadBalanceWeight.Value()
	dst.WANPrefixlen = aux.WANPrefixlen.Value()
	dst.WANSmartqDownRate = aux.WANSmartqDownRate.Value()
	dst.WANSmartqUpRate = aux.WANSmartqUpRate.Value()
	dst.WANVLAN = aux.WANVLAN.Value()
	dst.WireguardClientPeerPort = aux.WireguardClientPeerPort.Value()
...
func (dst *NetworkWANDHCPOptions) UnmarshalJSON(b []byte) error {
	...
	dst.OptionNumber = aux.OptionNumber.Value()
...
func (dst *NetworkWANProviderCapabilities) UnmarshalJSON(b []byte) error {
	...
	dst.DownloadKilobitsPerSecond = aux.DownloadKilobitsPerSecond.Value()
	dst.UploadKilobitsPerSecond = aux.UploadKilobitsPerSecond.Value()
