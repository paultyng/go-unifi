...
func (dst *HotspotPackage) UnmarshalJSON(b []byte) error {
	...
	dst.Hours = aux.Hours.Value()
	dst.Index = aux.Index.Value()
	dst.LimitDown = aux.LimitDown.Value()
	dst.LimitQuota = aux.LimitQuota.Value()
	dst.LimitUp = aux.LimitUp.Value()
	dst.TrialDurationMinutes = aux.TrialDurationMinutes.Value()
