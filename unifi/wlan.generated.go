...
func (dst *WLAN) UnmarshalJSON(b []byte) error {
	...
	dst.DTIM6E = aux.DTIM6E.Value()
	dst.DTIMNa = aux.DTIMNa.Value()
	dst.DTIMNg = aux.DTIMNg.Value()
	dst.GroupRekey = aux.GroupRekey.Value()
	dst.MinrateNaDataRateKbps = aux.MinrateNaDataRateKbps.Value()
	dst.MinrateNgDataRateKbps = aux.MinrateNgDataRateKbps.Value()
	dst.RoamClusterID = aux.RoamClusterID.Value()
	dst.SaeAntiClogging = aux.SaeAntiClogging.Value()
	dst.SaeGroups = make([]int, len(aux.SaeGroups))
	for i, v := range aux.SaeGroups {
		dst.SaeGroups[i] = int(v)
	}
	dst.SaeSync = aux.SaeSync.Value()
	dst.VLAN = aux.VLAN.Value()
	dst.WEPIDX = aux.WEPIDX.Value()
...
func (dst *WLANScheduleWithDuration) UnmarshalJSON(b []byte) error {
	...
	dst.DurationMinutes = aux.DurationMinutes.Value()
	dst.StartHour = aux.StartHour.Value()
	dst.StartMinute = aux.StartMinute.Value()
